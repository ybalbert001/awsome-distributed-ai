...
"""
Evaluate OpenRLHF GRPO checkpoint for language compliance.

This script is identical to the veRL evaluation script — OpenRLHF saves
checkpoints in HuggingFace format directly, so no conversion step is needed.

Usage (on a Ray worker pod with 4 GPUs):

    # Evaluate the GRPO model (checkpoints are already in HF format):
    python evaluate_gptoss.py \
        --model_path /fsx/openrlhf/checkpoints/grpo-gpt-oss-20b \
        --tp 4 \
        --output /fsx/experiments/openrlhf_grpo_eval.txt

    # Compare against the SFT baseline:
    python evaluate_gptoss.py \
        --model_path /fsx/models/gpt-oss-20b-sft-merged \
        --tp 4 \
        --output /fsx/experiments/sft_baseline_eval.txt

Requires: vllm, langdetect (both in the openrlhf-rlvr Docker image)
"""
...
# ---------------------------------------------------------------------------
# Test Prompts — 10 questions x 5 languages = 50 test cases
# ---------------------------------------------------------------------------
TEST_PROMPTS = {
    "English": [
        "What is 15% of 80?",
...
def generate_batch(llm, prompts: list, max_tokens: int = 512) -> list:
...
def run_evaluation(llm, output_file: str, max_tokens: int = 512):
    ...
    print(f"\n{'='*70}")
    print("OpenRLHF GRPO - LANGUAGE COMPLIANCE EVALUATION")
    print(f"{'='*70}")
...
def _save_results(results, stats, output_file):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("=" * 80 + "\n")
        f.write("OpenRLHF GRPO — LANGUAGE COMPLIANCE EVALUATION\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate OpenRLHF GRPO checkpoint for language compliance",
...
""",
    ...
    parser.add_argument("--model_path", required=True, help="Path to HF-format model")
    ...
    parser.add_argument("--output", type=str, default="/fsx/experiments/openrlhf_grpo_eval.txt", help="Output file")
