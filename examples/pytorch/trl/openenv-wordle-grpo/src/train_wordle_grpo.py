...
# Ensure src/ is on the path so imports work both in-repo and in containers
...
try:
    ...
    # When running inside the container, the package is installed directly
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-to",
        default="none",
        help="Logging integration to use: 'trackio', 'wandb', 'none', etc. (default: none).",
    )
    parser.add_argument(
        "--debug",
        action="store_true",
...
def rollout_once(
    ...
    for _turn in range(max_turns):
        ...
        repetition_score = scale_repetition_score(
            previous_occurrences, len(guess_counts)
        )
...
def main() -> None:
    ...
    default_output_dir = (
        Path("outputs") / f"wordle-grpo-{sanitize_name(args.model_id)}-{timestamp}"
    )
