...
```bash
cd awsome-distributed-ai/examples/6.stable-diffusion/single-node
# build the image
...
cd awsome-distributed-ai/6.stable-diffusion/multi-node
```
...
We provide a template YAML file for submitting the stable diffusion distributed training job in [3.stable-diffusion-eks.yaml-template](https://github.com/awslabs/awsome-distributed-ai/blob/stable-diffusion-eks/examples/6.stable-diffusion/multi-node/3.stable-diffusion-eks.yaml-template). You can substitute the environment variables in the template manifest as:
